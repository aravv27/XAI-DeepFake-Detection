"""
Class Label Mappings for ImageNet and COCO.

Provides human-readable class names for classification and segmentation models.
"""
...
def get_imagenet_labels() -> List[str]:
    ...
    """
    # Common ImageNet classes (subset for demo, full 1000 classes in production)
    # This is a simplified version with the most common classes
    labels = [
...
def get_coco_segmentation_labels() -> List[str]:
    """
    Get COCO/VOC segmentation labels (21 classes).
    
    These are the classes used by DeepLabV3+ pre-trained on COCO.
...
def get_imagenet_to_coco_mapping() -> Dict[str, str]:
    """
    Map ImageNet class names to COCO segmentation classes.
    
    This is used to find corresponding segmentation class for classification output.
    
    Returns:
        Dictionary mapping ImageNet class patterns to COCO class names
    """
    mapping = {
        # Dogs
        "retriever": "dog",
...
def find_matching_coco_class(imagenet_class: str) -> str:
    """
    Find matching COCO segmentation class for an ImageNet class.
    
    Args:
        imagenet_class: ImageNet class name
        
    Returns:
        Matching COCO class name or 'unknown'
    """
    mapping = get_imagenet_to_coco_mapping()
    imagenet_lower = imagenet_class.lower().replace("-", "_")
    
    # Direct match
    if imagenet_lower in mapping:
        return mapping[imagenet_lower]
    
    # Partial match
    for key, value in mapping.items():
        if key in imagenet_lower:
            return value
